(function(){
  ...
  function addStyles(){
    ...
    style.textContent=`
      ...
      .button.button-primary,.family-service-card .button.button-primary{color:#fff!important;background:#23A85D!important;border:2px solid #050505!important;box-shadow:0 4px 9px rgba(5,5,5,.28)!important;cursor:pointer!important;pointer-events:auto!important}
      ...
      .button.button-secondary{color:#082A47!important;background:#dceef8!important;border:2px solid #050505!important;box-shadow:0 4px 9px rgba(5,5,5,.22)!important;cursor:pointer!important;pointer-events:auto!important}
  ...
  function pairEmails(){
    ...
    const pair=document.createElement("div");pair.className="email-pair";grid.insertBefore(pair,a);pair.append(a,b);
  }

  function normalizeButtons(){
    const form=document.querySelector("#data-form");
    form?.querySelector('[data-action="back-step"]')?.remove();
    const dataSubmit=form?.querySelector('button[type="submit"]');if(dataSubmit)dataSubmit.textContent="Continuar";
    const eligibility=document.querySelector("#eligibility-form");eligibility?.querySelector('[data-action="back-step"]')?.remove();
    const pay=document.querySelector("#payment-form");const payBack=pay?.querySelector('[data-action="back-step"]');if(payBack)payBack.textContent="Modificar datos";const paySubmit=pay?.querySelector('button[type="submit"]');if(paySubmit)paySubmit.textContent="Informar pago";
  ...
  function tidyPayment(){
    const payment=document.querySelector(".payment-access-v2");if(!payment)return;
    const cards=payment.querySelectorAll(".payment-method-card");
    if(cards[0]){const h=cards[0].querySelector("h3");if(h)h.textContent="Pago por QR";}
    if(cards[1]){const h=cards[1].querySelector("h3");if(h)h.textContent="Pago por transferencia";}
  }

  function showExistingFiles(){
    const form=document.getElementById("data-form");const request=activeRequest();if(!form||!request?.answers)return;
    form.querySelectorAll('input[type="file"][name]').forEach(input=>{
      if(input.parentElement?.querySelector(".existing-file-note"))return;
      const stored=request.answers[input.name];if(!stored?.name)return;
      const note=document.createElement("small");note.className="existing-file-note";note.innerHTML=`Archivo ya cargado: <strong>${String(stored.name).replace(/[<>]/g,"")}</strong>. Si elegís otro, lo reemplaza.`;input.insertAdjacentElement("afterend",note);
    });
  }

  function saveDraft(form){
    const key=draftKey();if(!key||form?.id!=="data-form")return;
    const data={};form.querySelectorAll("input,select,textarea").forEach(el=>{if(!el.name||el.type==="file"||el.type==="password")return;if(el.type==="checkbox")data[el.name]=el.checked;else if(el.type==="radio"){if(el.checked)data[el.name]=el.value;}else data[el.name]=el.value;});
    try{sessionStorage.setItem(key,JSON.stringify(data));}catch(_){}
  }

  function restoreDraft(){
    const form=document.getElementById("data-form"),key=draftKey();if(!form||!key||form.dataset.draftRestored==="true")return;form.dataset.draftRestored="true";
    const data=readJSON(sessionStorage,key,null);if(!data||typeof data!=="object")return;let restored=false;
    Object.entries(data).forEach(([name,value])=>{const field=form.elements.namedItem(name);if(!field)return;if(field instanceof RadioNodeList){[...field].forEach(el=>{if(el.type==="radio")el.checked=String(el.value)===String(value);});restored=true;}else if(field.type==="checkbox"){field.checked=Boolean(value);restored=true;}else if(!field.value&&value){field.value=String(value);restored=true;}});
    if(restored&&!form.querySelector(".draft-restored-note")){const note=document.createElement("div");note.className="draft-restored-note";note.textContent="Recuperamos los datos que habías escrito en esta sesión.";form.querySelector(".form-grid")?.prepend(note);}
  }

  function clearDraftWhenSaved(){
    if(!document.getElementById("payment-form"))return;const key=draftKey();if(key)try{sessionStorage.removeItem(key);}catch(_){}
  }

  function setValidity(input,message){input.setCustomValidity(message||"");if(message)input.classList.add("user-touched");}

  function validateField(input){
    if(!input?.name)return true;const raw=String(input.value||"").trim(),digits=raw.replace(/\D/g,"");let message="";
  ...
  function validateBasics(form){
    if(!form||!["data-form","correction-form"].includes(form.id))return true;let ok=true;form.querySelectorAll("input,select,textarea").forEach(el=>{if(!validateField(el))ok=false;});
    const email=form.querySelector('input[name="email"]'),confirm=form.querySelector('input[name="emailConfirm"]');if(email&&confirm&&email.value.trim()&&confirm.value.trim()&&email.value.trim().toLowerCase()!==confirm.value.trim().toLowerCase()){setValidity(confirm,"Los correos electrónicos no coinciden.");ok=false;}return ok;
  }
  ...
  function showInvalidSubmit(form){
    ...
    if(first){first.scrollIntoView({behavior:"smooth",block:"center"});setTimeout(()=>{try{first.focus({preventScroll:true});}catch(_){first.focus();}form.reportValidity();},180);}else form.reportValidity();
    return true;
  }

  function simplifyStepper(){
    const stepper=document.querySelector(".stepper");if(!stepper||stepper.dataset.auditReady==="true")return;const steps=[...stepper.querySelectorAll(".step")];const labels=["Datos","Pago","Finalización"];if(steps.length===3)steps.forEach((s,i)=>{const l=s.querySelector("span:last-child");if(l)l.textContent=labels[i];});stepper.dataset.auditReady="true";
  }

  function simplifyTimeline(){
    const result=document.querySelector(".tracking-result"),timeline=result?.querySelector(".timeline"),badge=result?.querySelector(".status-badge");if(!timeline||timeline.dataset.auditReady==="true")return;
    const text=(badge?.textContent||"").toLowerCase();let current=0;if(/pago en revisión/.test(text))current=1;else if(/en proceso|falta información|listo para entregar/.test(text))current=2;else if(/finalizado/.test(text))current=3;const labels=["Solicitud","Pago","En gestión","Finalizado"];
    timeline.innerHTML=labels.map((label,i)=>`<div class="timeline-item ${i<current?"done":""} ${i===current?"current":""}"><span class="timeline-dot" aria-hidden="true"></span><div><strong>${label}</strong>${i===current?"<small>Estado actual</small>":""}</div></div>`).join("");timeline.dataset.auditReady="true";
  }

  function linkOpinion(){const link=document.querySelector(".final-opinion-cta a"),code=document.querySelector(".tracking-result .status-header .eyebrow")?.textContent?.trim();if(link&&code)link.href="opiniones.html?codigo="+encodeURIComponent(code);}

  function friendlyErrors(){document.querySelectorAll(".form-error").forEach(error=>{if(/quota|exceeded|storage|almacenamiento/i.test(error.textContent||""))error.textContent="No pudimos guardar el archivo en este dispositivo. Probá con una imagen más liviana o volvé a seleccionarla.";});}
  ...
  document.addEventListener("submit",e=>{const form=e.target;if(!validateBasics(form)){e.preventDefault();e.stopImmediatePropagation();showInvalidSubmit(form);}},true);
  document.addEventListener("click",e=>{
    const submit=e.target.closest?.('form button[type="submit"]');
    if(submit){const form=submit.closest("form");if(showInvalidSubmit(form)){e.preventDefault();e.stopImmediatePropagation();return;}}
    const back=e.target.closest?.('#payment-form [data-action="back-step"]');if(back){const receipt=document.querySelector('#payment-form input[name="receipt"]');if(receipt?.files?.length&&!window.confirm("El comprobante todavía no fue enviado. Si modificás los datos tendrás que seleccionarlo nuevamente. ¿Querés continuar?")){e.preventDefault();e.stopImmediatePropagation();return;}}
    if(e.target.closest?.("[data-error-retry]")){document.querySelector(".tramipago-error-box")?.remove();location.reload();}
  },true);
