...
const photosPath='assets/'+'abogado-tarjetas-fotos-20260917.css';
const asset=read(photosPath);
const page=read('legal-landing-20260917.css');
const js=read('legal-landing-20260917.js');
assert.ok(page.startsWith('@import url("'+photosPath),'La página carga las fotos recortadas');
...
assert.equal(createHash('sha256').update(photo).digest('hex'),'ba346b170b96cf306cfc20d6fb8e0bbc3976f4598819a9e3cc3fdcd1819e5987','Las fotografías no coinciden con los recortes de origen');
...
console.log('PASS: imagen WebP íntegra; cuatro recortes diferenciados, botones y formulario conectados.');
console.log('Límite: verificación de código e imagen; no es una inspección visual del navegador.');
