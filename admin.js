(async function(){
  "use strict";
  console.warn("TramiPago Admin: MODO DESARROLLO. MFA temporalmente desactivado; reactivarlo antes de producción.");
  try{
    const response=await fetch("admin-mfa.js?v=20260915-dev1",{cache:"no-store"});
    ...
    loginPanel.hidden=true;dashboard.hidden=false;logoutButton.hidden=false;sessionLabel.textContent=(currentUser.email||"Administrador")+" · modo desarrollo sin MFA";await loadRequests();
  }

  async function loadRequests(){`);
    (0,eval)(source+"\n//# sourceURL=admin-dev-runtime.js");
  }catch(error){
    console.error("TramiPago Admin modo desarrollo:",error);
    const node=document.getElementById("login-message");
    if(node){node.textContent="No se pudo iniciar el panel en modo de desarrollo.";node.className="message error";}
  }
})();