(async function(){
  ...
  const FILE_KIND_LABELS={payment_receipt:"Comprobante de pago",dni:"Documento / DNI",supporting_document:"Documento adjunto",result:"Resultado",other:"Archivo"};
  ...
  const dateTime=value=>{if(!value)return "—";try{return new Intl.DateTimeFormat("es-AR",{dateStyle:"short",timeStyle:"short"}).format(new Date(value));}catch(_){return String(value);}};
  const money=value=>value===null||value===undefined||value===""?"—":new Intl.NumberFormat("es-AR",{style:"currency",currency:"ARS",maximumFractionDigits:0}).format(Number(value));
  const fileSize=value=>{const n=Number(value);if(!Number.isFinite(n)||n<=0)return "—";if(n<1024)return `${n} B`;if(n<1048576)return `${(n/1024).toFixed(1)} KB`;return `${(n/1048576).toFixed(1)} MB`;};
  const setMessage=(node,text,type="")=>{node.textContent=text||"";node.className=`message${type?` ${type}`:""}`;};
  const serviceConfig=id=>(window.TRAMI_SERVICES||[]).find(service=>service.id===id)||null;
  const serviceName=row=>{const related=Array.isArray(row.services)?row.services[0]:row.services;return related?.name||serviceConfig(row.service_id)?.name||row.service_id;};
  const fieldConfig=(row,key)=>serviceConfig(row.service_id)?.fields?.find(field=>field.id===key)||null;
  ...
  function payloadGrid(row,detail){
    const rd=Array.isArray(detail.request_data)?detail.request_data[0]:detail.request_data;
    const payload=rd?.payload&&typeof rd.payload==="object"?rd.payload:{};
    ...
    return `<div class="record-grid">${entries.map(([key,value])=>{
      const field=fieldConfig(row,key);
      return `<div class="record-item"><strong>${escapeHTML(field?.label||humanizeKey(key))}</strong><span>${escapeHTML(valueLabel(row,key,value))}</span></div>`;
    }).join("")}</div>`;
  }

  function fileList(detail){
    const files=Array.isArray(detail.request_files)?[...detail.request_files].sort((a,b)=>new Date(a.created_at)-new Date(b.created_at)):[];
    if(!files.length)return '<p class="muted">No hay archivos adjuntos registrados.</p>';
    return `<div class="file-list">${files.map(file=>`<div class="file-row"><div><strong>${escapeHTML(FILE_KIND_LABELS[file.kind]||file.kind)}</strong><span>${escapeHTML(file.original_name||"Archivo")}</span><small>${escapeHTML(file.mime_type||"Tipo no informado")} · ${escapeHTML(fileSize(file.size_bytes))} · ${escapeHTML(dateTime(file.created_at))}</small></div><button class="btn secondary" type="button" data-action="open-file" data-path="${escapeHTML(file.storage_path)}">Abrir archivo</button></div>`).join("")}</div>`;
  }

  function eventList(detail){
    const events=Array.isArray(detail.request_events)?[...detail.request_events].sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)):[];
    if(!events.length)return '<p class="muted">Todavía no hay movimientos registrados.</p>';
    return `<div class="history">${events.map(event=>`<div class="history-row"><strong>${escapeHTML(STATUS_LABELS[event.status]||event.event_type||"Movimiento")}</strong><span>${escapeHTML(dateTime(event.created_at))}</span>${event.note?`<p>${escapeHTML(event.note)}</p>`:""}</div>`).join("")}</div>`;
  }

  function controlData(row){
    const items=[
      ["Código",row.tracking_code],["Servicio",serviceName(row)],["Estado",STATUS_LABELS[row.status]||row.status],
      ["Cliente",row.client_name],["Correo",row.email],["WhatsApp",row.whatsapp],["Importe",money(row.quoted_amount)],
      ["Formulario",dateTime(row.form_completed_at)],["Comprobante",dateTime(row.receipt_uploaded_at)],["Pago confirmado",dateTime(row.paid_at)],
      ["Inicio",dateTime(row.started_at)],["Finalizado",dateTime(row.finalized_at)],["Fecha estimada",dateTime(row.estimated_completion_at)],
      ["Última actualización",dateTime(row.updated_at)]
    ];
  ...
  function detailHTML(row,detail){
    return `<section class="record-section"><h3>Datos del formulario</h3>${payloadGrid(row,detail)}</section><section class="record-section"><h3>Archivos y comprobantes</h3>${fileList(detail)}</section><section class="record-section"><h3>Datos de control</h3>${controlData(row)}</section><section class="record-section"><h3>Historial</h3>${eventList(detail)}</section>`;
  }
  ...
  async function loadRequests(){
    setMessage(dashboardMessage,"Cargando trámites…");
    dashboard.classList.add("spinner");
    detailCache.clear();
    try{
      const {data,error}=await supabase.from("requests").select(`id,tracking_code,service_id,status,client_name,email,whatsapp,quoted_amount,status_note,estimated_completion_at,form_completed_at,receipt_uploaded_at,paid_at,started_at,finalized_at,created_at,updated_at,services(name)`).order("created_at",{ascending:false}).limit(300);
      if(error)throw error;
      rows=data||[];render();setMessage(dashboardMessage,"");
    }catch(error){console.error(error);setMessage(dashboardMessage,"No se pudieron cargar los trámites.","error");}
  ...
  function filteredRows(){
    ...
    return rows.filter(row=>{
      ...
      if(!term)return true;
      return [row.tracking_code,row.client_name,row.whatsapp,row.email,row.service_id,serviceName(row)].some(value=>String(value||"").toLowerCase().includes(term));
    });
  }
  ...
  async function loadDetail(requestId,cardNode,button){
    const holder=cardNode.querySelector(".lazy-detail");
    if(detailCache.has(requestId)){
      const row=rows.find(item=>item.id===requestId);
      holder.innerHTML=detailHTML(row,detailCache.get(requestId));holder.hidden=false;button.textContent="Ocultar ficha";button.dataset.action="hide-detail";return;
    }
    const original=button.textContent;button.disabled=true;button.textContent="Cargando…";
    try{
      const {data,error}=await supabase.from("requests").select(`id,request_data(payload,updated_at),request_files(id,kind,storage_path,original_name,mime_type,size_bytes,created_at),request_events(id,event_type,status,note,created_by,created_at)`).eq("id",requestId).single();
      if(error)throw error;
      detailCache.set(requestId,data);
      const row=rows.find(item=>item.id===requestId);
      holder.innerHTML=detailHTML(row,data);holder.hidden=false;button.textContent="Ocultar ficha";button.dataset.action="hide-detail";
    }catch(error){console.error(error);alert("No se pudo cargar la ficha completa.");button.textContent=original;}
    finally{button.disabled=false;}
  }

  async function saveRow(form,requestId){
    const message=form.querySelector(".message");setMessage(message,"Guardando…");
    ...
    if(!EDITABLE_STATUSES.includes(status)){setMessage(message,"Elegí uno de los cuatro estados de gestión.","error");return;}
    if(status==="needs_info"&&!note){setMessage(message,"Indicá qué información necesita corregir el cliente.","error");return;}
    ...
    if(error){console.error(error);setMessage(message,"No se pudieron guardar los cambios.","error");return;}
    setMessage(message,"Cambios guardados.","ok");await loadRequests();
  }

  async function openPrivateFile(button){
    const path=button.dataset.path;if(!path)return;
    const original=button.textContent;button.disabled=true;button.textContent="Abriendo…";
    try{const {data,error}=await supabase.storage.from(STORAGE_BUCKET).createSignedUrl(path,300);if(error)throw error;window.open(data.signedUrl,"_blank","noopener,noreferrer");}
    catch(error){console.error(error);alert("No se pudo abrir el archivo privado.");}
  ...
  requestsNode.addEventListener("click",async event=>{
    ...
    if(button.dataset.action==="open-file"){await openPrivateFile(button);return;}
    ...
    if(button.dataset.action==="close"){editor.hidden=true;return;}
    if(button.dataset.action==="choose-status"){editor.elements.status.value=button.dataset.status||"";editor.querySelectorAll(".status-choice").forEach(item=>item.classList.toggle("active",item===button));}
  });

  requestsNode.addEventListener("submit",async event=>{const form=event.target;if(!(form instanceof HTMLFormElement)||!form.classList.contains("editor"))return;event.preventDefault();const requestId=form.closest(".card")?.dataset.requestId;if(requestId)await saveRow(form,requestId);});
